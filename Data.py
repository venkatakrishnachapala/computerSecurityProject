...
from Payload import set_stats_tracker
...
class AttackStats:
    def __init__(self):
        self.stats = {
            "Reflected XSS": {"attempts": 0, "successes": 0},
            "Stored XSS": {"attempts": 0, "successes": 0},
            "HTML Injection": {"attempts": 0, "successes": 0},
            "SQL Injection": {"attempts": 0, "successes": 0},
        }

    def record_attempt(self, attack_type: str):
        if attack_type in self.stats:
            self.stats[attack_type]["attempts"] += 1

    def record_success(self, attack_type: str):
        if attack_type in self.stats:
            self.stats[attack_type]["successes"] += 1

    def generate_report(self):
        print("\n==============================")
        print("Attack Statistics Report:")
        total_tested = 0
        total_success = 0

        for attack, stats in self.stats.items():
            tested = stats["attempts"]
            success = stats["successes"]
            rate = (success / tested * 100) if tested else 0.0
            total_tested += tested
        ...
        overall_rate = (total_success / total_tested * 100) if total_tested else 0.0
        print("\nOverall Stats:")
...
def print_vulnerabilities_by_attack_type(context: Context, xss_found: list[Found], tokens_found: list[Found]) -> None:
    """Print discovered vulnerabilities grouped by attack type"""
    attacks = {
        "Reflected XSS": [],
        "Stored XSS": [],
        "HTML Injection": [],
    ...
    all_found = xss_found + tokens_found

    for id, url, scan in all_found:
        if id in context:
            attack_type = getattr(context[id], 'attack_type', None)
            if attack_type in attacks:
                attacks[attack_type].append((id, url, scan))

    write_msg("\n=== Vulnerabilities by Attack Type ===")
    for attack, entries in attacks.items():
        write_msg(f"\n{attack} ({len(entries)}):")
        for id, url, scan in entries:
            db_loc = context[id] if id in context else "???"
            write_msg(f"  ID={id}, URL={url}, DB={db_loc}, Scan={scan}")
    write_msg("=== End of Summary ===\n")

def look_for(context: Context, xss_found: list[Found], tokens_found: list[Found], scan_index: int,
             look_id: Optional[int], look_table: Optional[str],
             look_column: Optional[str], look_row: Optional[str]) -> None:
    """Look for reflections that match a specific id, table, column, or row hint."""
    write_msg("\nManual Look-for Results:")

    def match(entry_id: int) -> bool:
        if look_id is not None and entry_id != look_id:
            return False
        db_entry = context.get(entry_id)
        if db_entry is None:
            return False
        if look_table and look_table not in db_entry.table:
            return False
        if look_column and look_column not in db_entry.column:
            return False
        if look_row and look_row not in db_entry.row:
            return False
        return True

    found_any = False
    for label, found_set in [('XSS', xss_found), ('Token', tokens_found)]:
        for entry_id, url, scan in found_set:
            if scan != scan_index + 1:
                continue
            if match(entry_id):
                found_any = True
                db_loc = context.get(entry_id, "???")
                write_msg(f"[{label}] ID={entry_id}, Scan={scan}, URL={url}, DB={db_loc}")

    if not found_any:
        write_msg("No matching results found.")
