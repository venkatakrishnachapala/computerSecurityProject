...
class BlackWidowScanner(Scanner):
    ...
    def scan(self, urls, tokens=None, timeout=30):
        ...
        if tokens:
            # Save to temporary token file for compatibility
            fp_tokens = tempfile.NamedTemporaryFile(delete=False, suffix='.txt', mode='w', encoding='utf-8')
            for token in tokens:
                if token:
                    fp_tokens.write(token + "\n")
            fp_tokens.close()

            # Create a CSV-formatted payload file (if crawl.py expects --payload_csv)
            payload_csv = tempfile.NamedTemporaryFile(delete=False, suffix='.csv', mode='w', encoding='utf-8')
            payload_csv.write("id,payload\n")
            for i, token in enumerate(tokens):
                payload_csv.write(f"{i},\"{token}\"\n")
            payload_csv.close()

            # Send both to BW
            bw_cmd += ["--tokens_file", fp_tokens.name]
            bw_cmd += ["--payload_csv", payload_csv.name]
    ...
    def initialScan(self, urls: list[str], timeout: int = 300) -> list[str]:
        ...
        bw_cmd = ['python', '-u', BW, '--shallow', url]
