...
from Data import print_vulnerabilities_by_attack_type
...
def main(args: Namespace) -> None:
    ...
    for index in tqdm(indices, disable=not WRITE_MESSAGES):
        ...
        if not db.is_row_in_table(table, row, col_info, key_info, primary=app_config.primary_keys):
            ...
            # Track success if our payload is in the value
            attack_type = getattr(Payload.context.get(latest, None), "attack_type", None)
            if attack_type and any(tag in str(new[k]).lower() for tag in ["<script>", "xss(", "alert(", "onerror="]):
                stats.record_attempt(attack_type)
                stats.record_success(attack_type)
        ...
        if index in reflection_indices:  # need to scan even if not fuzzed (at this index)
            ...
            payload_tokens = []
            try:
                df = pd.read_csv("Generated_Test_Payloads.csv")  # Or full path if needed
                payload_tokens = df['payload'].dropna().tolist()
            except Exception as e:
                write_msg(f"⚠️ Failed to load payload tokens: {e}")
            reflections = scanner.scan(reflection_urls, tokens=payload_tokens, timeout=app_config.scanner_timeout)
            scans += 1
                ...
                for r in reflections:
                    ...
                    if 'xss_ids' in r:
                        ...
                        for xss_id in xss_ids:
                            ...
                            # Increment type-specific stats from Payload.context
                            attack_type = getattr(Payload.context.get(xss_id, None), "attack_type", None)
                            if attack_type:
                                stats.record_attempt(attack_type)
                                stats.record_success(attack_type)
                    ...
                    if 'token_ids' in r:
                        ...
                        for token_id in token_ids:
                            ...
                            attack_type = getattr(Payload.context.get(token_id, None), "attack_type", None)
                            if attack_type:
                                stats.record_attempt(attack_type)
                                # tokens don't imply success for stored/reflected unless detected, so we skip `.record_success`
...
if __name__ == "__main__":
    ...
    if Path(args.print).exists():
        ...
        print_vulnerabilities_by_attack_type(context, xss_found, tokens_found)
